<template>
    ...
    <div class="title">
      <div>
        <span v-for="item,index in type1" :key='index'  :class="actve==item.num?'actve':'0'" @click="choose(item.num,index)">{{$store.state.lg=='C'?item.name:item.ename}}</span>
      </div>
      <div>
        <span @click="book(1)" style="background: firebrick;">+ {{$store.state.lg=='C'?'买涨':'Buy up'}}</span>
        <span @click="book(2)" style="background: green;">+ {{$store.state.lg=='C'?'买跌':'Buy put'}}</span>
      </div>
    </div>
    <div class="kline"  id='maink'>
       <div v-show="actve=='a'">
          <div id="echartContainermin" ref="echartContainermin"  style="width:100%; height:600px"></div>
       </div>
       <div  v-show="actve=='b'">
          <div id="echartContainer15" ref="echartContainer15"  style="width:100%; height:600px"></div>
       </div>
       <div  v-show="actve=='c'">
          <div id="echartContainer30" ref="echartContainer30"  style="width:100%; height:600px"></div>
       </div>
       <div  v-show="actve=='d'">
         	<div id="echartContainer60" ref="echartContainer60"  style="width:100%; height:600px"></div>
       </div>
       <div  v-show="actve=='e'">
         <div id="echartContainerday" ref="echartContainerday"  style="width:100%; height:600px"></div>
       </div>
    </div>
    ...
    <div class="main" v-show="viewshow">
    	<p>{{$store.state.lg=='C'?'签署交易协议':'Signing of transaction agreement'}}</p>
    ...
    <div class="buy" v-show="showbuy">
      <div class="title">
         <p>
           <span>{{$store.state.lg=='C'?$store.state.detaillist.productName:$store.state.detaillist.eproductName}}</span>
         </p>
      ...
      <div class="money">
        ...
        <div class="btn" @click="surebuy">
          ...
          <!-- <button style="background: green;">{{$store.state.lg=='C'?'买跌':'Buy put'}}</button> -->
        </div>
      </div>
    </div>
 </div>
...
export default {
  ...
  data () {
    return {
      ...
      ss:['1','2','3','5','10'],
       ...
       shousu:'',
       showbuy:false,
      type1:[{ name:'分时',ename:'Timesharing', num:'a',},{name:'15分',ename:'15 minutes', num:'b',},{ name:'30分',ename:'30 minutes', num:'c',},{name:'1小时',ename:'An hour', num:'d',},{name:'日k',ename:'Day',num:'e',}],
      a:5,
      b:30,
      option: null,
       mindata:'',
  ...
  methods:{
    ...
    if(this.$store.state.info==''){
      this.$message.warning(this.$store.state.lg=='C'?'请先登录':'Please log in first');
    ...
    colsebuy(){
      ...
      surebuy() {
          this.$confirm(this.$store.state.lg=='C'?'是否确定买入?':'Are you sure to buy', this.$store.state.lg=='C'?'提示':'Tips', {
...
//     y: "90%",
   ...
   let url='';
   val=='c'?url=this.testUrl+'product/getKLine30Minute':url=this.testUrl+'product/getKLineHourMinute'
